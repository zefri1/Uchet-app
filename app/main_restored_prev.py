...
def objects_page(request: Request, db: Session = Depends(get_db)):
    ...
    return render(request, "objects.html", db, objects=objects)
...
def create_rule(
    ...
    actual_tenant_id: int | None = None
...
def calculate_period(request: Request, period_id: int, db: Session = Depends(get_db)):
    try:
        ...
        return render_billing_page(request, db, selected_period_id=period_id, error=str(exc), status_code=400)
